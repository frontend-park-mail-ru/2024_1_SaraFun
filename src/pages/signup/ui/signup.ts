...
export class RegistrationPage {
   ...
   */
	addEventListeners(): void {
		...
		const step1 = document.querySelector('.step1') as HTMLElement;
		const step2 = document.querySelector('.step2') as HTMLElement;
		...
		if (nextButton) {
			nextButton.addEventListener('click', () => {
				...
				if (loginErrors.length > 0 || passwordErrors.length > 0 || !valid) {
		...
		const interestTags = document.querySelectorAll('.interest-tag') as NodeListOf<HTMLElement>;
		...
		const signupButton = document.querySelector('.signup-button') as HTMLElement;
		signupButton.addEventListener('click', async () => {
			...
			if (valid) {
				try {
					const isSignedUp = await signupUser(this.login, this.password, first_name, this.gender, this.birth_date);
					if (!isSignedUp) {
